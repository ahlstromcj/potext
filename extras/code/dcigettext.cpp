 ...
 * Loose ends:
 *
 *      -   struct loaded_I10nfile
 *      -    union { char appended[ZERO]; const char *ptr; }
 *
 * THIS MODULE IS CURRENT UNCOMPILABLE.
 *
 * We might, in the future, finish converting this module to C++, which
 * will cut out:
 *
 *  malloc(), free(), alloca(), freea(), strcmp(), strlen(), strdup(),
 *  jillions of pointers to characters and structures, checks for _LIBC,
 *  IN_LIBGLOCALE, __GLIBC__, _LIBICONV_VERSION, calls to lock/unlock functions
 *  like gl_rwlock_rdlock(), gl_rwlock_wrlock(), __libc_lock_lock() ....
 */
 ...
 *  are stored. It replaces struct known_translation_t.
 */

class known_translation
{
    /**
     *  The message domain in which to search.
     */
    ...
    /**
     *  The name of the relevant locale category, or "" for the global locale.
     */
    ...
    /**
     *  The character encoding for the translation.
     */
    ...
    /**
     *  State of the catalog counter at the point the where string was found.
     */

    int m_counter;

    /**
     *  Catalog where the string was found. This structure is defined in
     *  the glibc header loadinfo.h, and supports a linked list.
     */

    struct loaded_l10nfile * m_domain_ptr;
...
{
    int result;
    const known_translation * s1 = static_cast<known_translation *>(p1);
    const known_translation * s2 = static_cast<known_translation *>(p2);
    result = strcmp
    (
        s1->m_domain_ptr != NULL ? s1->msgid.appended : s1->msgid.ptr,
        s2->m_domain_ptr != NULL ? s2->msgid.appended : s2->msgid.ptr
    );
    if (result == 0)
    {
...
const std::string _nl_default_default_domain = "messages";
...
std::string _nl_current_default_domain = _nl_default_default_domain;
...
extern const std::string _nl_default_dirname;

libc_hidden_proto (_nl_default_dirname)

#endif

const std::string _nl_default_dirname = LOCALEDIR;
...
(
    ...
    unsigned long int n,
    const std::string &  translation    // , size_t translation_len
);
...
#endif
{
    ...
    std::string xdirname = NULL;
    std::string xdomainname;
    std::string single_locale;
    std::string retval;
    size_t retlen;
    int saved_errno;
    ...
    {
        /* Bogus.  Use the Germanic plural rule.  */

        return
        (
            plural == 0 ? msgid1 : n == 1 ? msgid1 : msgid2
        );
    }
#endif
...
# endif
    ...
    if (foundp != NULL && (*foundp)->counter == _nl_msg_cat_cntr)
    {
        ...
        if (plural)
        {
            ...
            (
                (*foundp)->domain, n, (*foundp)->translation //,
...
#endif      // #if defined HAVE_PER_THREAD_LOCALE
    ...
    for (binding = _nl_domain_bindings; binding != NULL; binding = binding->next)
    {
        ...
        if (compare < 0)
        {
            binding = NULL;             /* It is not in the list.  */
            break;
        }
    }
...
#if ! defined CATEGORYNAME_INITIALIZED
    categoryname = category_to_name (category);
#endif
#if defined IN_LIBGLOCALE
    ...
    xdomainname = categoryname + "/" + domainname + ".mo";

    /*
     *  Creating working area. We can use new here.
     */
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            *cp = '\0';
             ...
             * outside the dedicated directories. Ignore this entry.
             */
        ...
        if (retval != NULL)
        {
            ...
            if (foundp == NULL)
            {
                ...
                if (newp != NULL)
                {
                    ...
                    char *new_localename;
#endif
                    new_domainname = (char *) mempcpy
                        (newp->msgid.appended, msgid1, msgid_len);
    ...
    return_untranslated:            /* return the untranslated MSGID  */
    ...
    return (plural == 0 ? (char *) msgid1 : n == 1 ? (char *) msgid1 :
        (char *) msgid2);
}
...
#endif
{
    ...
    if (domain->hash_tab != NULL)           /* Use the hashing table.  */
    {
            ...
            (
                ...
                :
                    domain->orig_sysdep_tab[nstr - nstrings].length > len
                        && (strcmp (msgid, domain->orig_sysdep_tab[nstr - nstrings]
                            .pointer) == 0)
            )
            {
                act = nstr;
    ...
    else
    {
        ...
        while (bottom < top)
        {
            ...
            cmp_val = strcmp
            (
                msgid, (domain->data + W (domain->must_swap,
        ...
        return NULL;                    /* No translation was found.  */
    }
...
#define INITIAL_BLOCK_SIZE    4080
                ...
                for (;;)
                {
                    ...
                    res = __gconv (convd->conv, &inbuf, inbuf + resultlen,
                        &outbuf, outbuf + freemem_size - sizeof (size_t),
                        &non_reversible);
...
__intl_freemem (void)
{
    ...
    while (_nl_domain_bindings != NULL)
    {
        ...
        if (oldp->dirname != _nl_default_dirname) /* a pointer comparison.  */
            free (oldp->dirname);
